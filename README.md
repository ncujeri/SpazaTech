...
- **SaleBuilder** (Domain, pure logic): assembles the immutable sale graph from cart lines and tenders. Cash is never entered as a tender; it is always the rounded remainder, with the rounding difference recorded explicitly on the sale so cash-up balances. Unit costs are snapshotted onto every line at sale time. Voids are compensating sales referencing the original, and the cash refund mirrors the rounded amount the customer actually paid, not the raw total. Partial returns (BuildPartialReversal) bring back only the chosen lines and quantities, refunded by one method, with each reversal line linked to the original it returns so the remaining returnable quantity is always known.
- **Returns**: a customer bringing goods back is handled on the Returns screen (reachable from the till), where the cashier taps the sale, chooses how many of each line to return, and picks how to give the money back: cash from the drawer, a card or SASSA reversal, or a credit against the Makhulu Book. A full-sale refund is one tap and refunds every original tender by its own method. Returns move cash, so they are owner-only by default; the owner can switch them on for cashiers under Shift. Cash refunds are blocked when the drawer cannot cover them, with an explicit owner override. On-book sales have their ledger debit cancelled so a returned item is no longer owed.
